...
mkdir -p "${HOME}/.archon/.archon/workflows"
...
copy_managed_file "${REPO_ROOT}/templates/global/archon/config.yaml" "${HOME}/.archon/config.yaml"
copy_managed_dir "${REPO_ROOT}/templates/global/archon/.archon/workflows" "${HOME}/.archon/.archon/workflows"

copy_skill_set "${HOME}/.codex/skills"
copy_skill_set "${HOME}/.claude/skills"
copy_skill_set "${HOME}/.agents/skills"

log "Global Codex, Claude, Archon and skill setup synchronized."
